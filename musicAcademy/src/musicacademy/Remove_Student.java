...
public class Remove_Student implements ActionListener{
    ...
    public Remove_Student() {
        ...
        l5.setLayout(null);
        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("musicacademy/icons/remove.jpg"));
        l5.setIcon(img);
        f.add(l5);
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b){
            try{
                ...
                if(rs.next()){
                    ...
                    b1.setVisible(true);
                    b2.setVisible(true);    
                    i=1;//if the id is valid then change i to 1
                    l6.setText(name);
                    l7.setText(mob);
                    l8.setText(email);
                }
                if(i==0)//if the id not found then i will remain 0 only
        ...
        if(ae.getSource()==b1){
            try{
                ...
                con.s.executeUpdate(str);//executeUpdate updates the database
                JOptionPane.showMessageDialog(null,"deleted successfully");
                l2.setVisible(false);//Setting the six labels visibility to false so that they are not visible after deletion
                l3.setVisible(false);
                l4.setVisible(false);
                l6.setVisible(false);
                l7.setVisible(false);
        ...
        if(ae.getSource()==b2){//if we press cancel(b2) then details class is called
            f.setVisible(false);
