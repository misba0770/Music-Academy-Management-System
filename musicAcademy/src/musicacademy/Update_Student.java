...
public class Update_Student extends Add_Student implements ActionListener {
    ...
    Update_Student(String idaa){
        ...
        id15.setLayout(null);
        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("musicacademy/icons/add_student.jpg"));
        id15.setIcon(img);
    ...
    void showData(String s){
        try{
            ...
            if(i==0)//to check if id is present in the database to be updated or not
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b && i==1){
            try{
                conn con = new conn();
                String str = "update student set name='"+t1.getText()+"',fname='"+t2.getText()+"',age='"+age+"',dob='"+dat+"',address='"+t3.getText()+"',phone='"+t4.getText()+"',email='"+t5.getText()+"',instrument='"+t6.getText()+"',level='"+t7.getText()+"',batch='"+t8.getText()+"',student_id='"+t9.getText()+"' where student_id='"+id_student+"'";
                con.s.executeUpdate(str);
                JOptionPane.showMessageDialog(null,"successfully updated");//pop up
                f.setVisible(false);
